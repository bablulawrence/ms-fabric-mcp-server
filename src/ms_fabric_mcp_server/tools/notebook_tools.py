...
def register_notebook_tools(mcp: "FastMCP", notebook_service: FabricNotebookService):
    ...
    def get_notebook_run_details(
        workspace_name: str, notebook_name: str, job_instance_id: str
    ) -> dict:
    ...
    def list_notebook_runs(
        workspace_name: str, notebook_name: str, limit: Optional[int] = None
    ) -> dict:
        ...
        **Use this tool when:**
        ...
        result = notebook_service.list_notebook_runs(
            workspace_name=workspace_name, notebook_name=notebook_name, limit=limit
        )

        if result.get("status") == "success":
            logger.info(
                f"Found {result.get('total_count', 0)} runs for notebook '{notebook_name}'"
            )
        else:
