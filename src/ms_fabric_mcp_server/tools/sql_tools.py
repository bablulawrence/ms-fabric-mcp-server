...
def register_sql_tools(mcp: "FastMCP", sql_service: FabricSQLService):
    ...
    def get_sql_endpoint(
        workspace_name: str, item_name: str, item_type: str = "Warehouse"
    ) -> dict:
        ...
        logger.info(
            f"Getting SQL endpoint for {item_type} '{item_name}' in workspace '{workspace_name}'"
        )

        connection_string = sql_service.get_sql_endpoint(
        ...
        logger.info(
            f"Successfully retrieved connection string for {item_type} '{item_name}'"
        )
        return result
    ...
    def execute_sql_query(
        sql_endpoint: str, query: str, database: str = "Metadata"
    ) -> dict:
        ...
        **Note**: This tool is for queries that return data (SELECT/SHOW/DESCRIBE).
        ...
        logger.info(
            f"Executing SQL query on {sql_endpoint} (database: {database}): {query[:100]}..."
        )

        result = sql_service.execute_sql_query(
            sql_endpoint=sql_endpoint, query=query, database=database
        )
    ...
    def execute_sql_statement(
        ...
        **Note**: This tool is for DML statements that modify data (INSERT/UPDATE/DELETE/MERGE).
        ...
        logger.info(
            f"Executing DML statement on {sql_endpoint} (database: {database}): {statement[:100]}..."
        )
        ...
        if result["status"] == "success":
            logger.info(
                f"DML statement executed successfully: {result['affected_rows']} rows affected"
            )
        else:
