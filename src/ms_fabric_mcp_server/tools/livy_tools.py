...
def register_livy_tools(mcp: "FastMCP", livy_service: FabricLivyService):
    ...
    @handle_tool_errors
    def livy_list_sessions(workspace_id: str, lakehouse_id: str) -> dict:
        ...
        """
        log_tool_invocation(
            "livy_list_sessions", workspace_id=workspace_id, lakehouse_id=lakehouse_id
        )
        logger.info(
        ...
        try:
            ...
            logger.info(
                f"Successfully retrieved {len(result.get('sessions', []))} Livy sessions"
            )
            return result
    ...
    def livy_get_session_status(
        workspace_id: str, lakehouse_id: str, session_id: str
    ) -> dict:
    ...
    def livy_run_statement(
        ...
        try:
            ...
            logger.error(
                f"Unexpected error running Livy statement: {exc}", exc_info=True
            )
            return {
                "status": "error",
    ...
    def livy_get_statement_status(
        workspace_id: str, lakehouse_id: str, session_id: str, statement_id: str
    ) -> dict:
        ...
        logger.debug(
            f"Getting status for statement {statement_id} in session {session_id}"
        )
    ...
    def livy_get_session_log(
        ...
        **Note**: Returns driver-side logs only, not executor logs.
        ...
        logger.debug(
            f"Getting logs for session {session_id} (start={start}, size={size})"
        )
