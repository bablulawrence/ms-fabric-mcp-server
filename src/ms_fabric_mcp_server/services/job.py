...
class FabricJobService:
    ...
    def run_on_demand_job(
        ...
        try:
            ...
            response = self.client.make_api_request(
                "POST", url, payload=payload if payload else None, timeout=60
            )
            ...
            return RunJobResult(status="error", message=str(exc))
        except Exception as exc:
            logger.error(f"Unexpected error running job: {exc}")
            return RunJobResult(status="error", message=f"Unexpected error: {exc}")
    ...
    def get_job_status(
        ...
        try:
            ...
            return JobStatusResult(
                status="success", job=job, message="Job status retrieved successfully"
            )

        except (FabricItemNotFoundError, FabricAPIError) as exc:
            logger.error(f"Failed to get job status: {exc}")
            return JobStatusResult(status="error", message=str(exc))
        except Exception as exc:
            logger.error(f"Unexpected error getting job status: {exc}")
            return JobStatusResult(status="error", message=f"Unexpected error: {exc}")
    ...
    def get_job_status_by_url(self, location_url: str) -> JobStatusResult:
        ...
        try:
            ...
            if (
                ...
                return JobStatusResult(status="error", message="Invalid job status URL")
    ...
    def get_operation_result(self, operation_id: str) -> OperationResult:
        ...
        try:
            ...
            return OperationResult(
                status="error", operation_id=operation_id, message=str(exc)
            )
        except Exception as exc:
    ...
    def run_notebook_job(
        ...
        if run_result.status == "error":
            return JobStatusResult(status="error", message=run_result.message)
        ...
        if run_result.location_url:
            ...
            return JobStatusResult(
                status="error", message="No location URL returned from job creation"
            )
