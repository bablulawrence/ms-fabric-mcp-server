...
class FabricWorkspaceService:
    ...
    def get_workspace_by_id(self, workspace_id: str) -> FabricWorkspace:
        ...
        try:
            ...
            logger.info(
                f"Found workspace '{workspace.display_name}' with ID: {workspace_id}"
            )
            return workspace
    ...
    def create_workspace(
        ...
        try:
            response = self.client.make_api_request(
                "POST", "workspaces", payload=payload, timeout=60
            )
            ...
            logger.info(
                f"Successfully created workspace '{display_name}' with ID: {workspace.id}"
            )
            return workspace
    ...
    def delete_workspace(self, workspace_identifier: str) -> None:
        ...
        try:
            self.client.make_api_request(
                "DELETE", f"workspaces/{workspace_id}", timeout=60
            )
