...
from ..client.exceptions import (FabricConnectionError, FabricError,
...
class FabricSQLService:
    ...
    def get_sql_endpoint(
        self, workspace_name: str, item_name: str, item_type: str = "Warehouse"
    ) -> str:
    ...
    def get_warehouse_sql_endpoint(
        self, workspace_name: str, warehouse_name: str
    ) -> str:
                ...
                if attempt == 0 and self._is_transient_odbc_error(exc):
                    ...
                    if sql_endpoint and database:
                        try:
                            ...
                            logger.warning(f"Retry reconnect failed: {reconnect_exc}")
                    continue
    ...
    def execute_statement(
        self, statement: str, allow_ddl: bool = False
    ) -> Dict[str, Any]:
    ...
    def _is_transient_odbc_error(exc: Exception) -> bool:
        ...
        for arg in getattr(exc, "args", ()) or ():
            if isinstance(arg, str) and any(
                code in arg.lower() for code in transient_codes
            ):
                return True
        return False

    def execute_sql_query(
        self, sql_endpoint: str, query: str, database: str = "Metadata"
    ) -> QueryResult:
