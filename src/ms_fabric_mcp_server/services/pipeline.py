...
class FabricPipelineService:
    ...
    def _validate_pipeline_inputs(
        ...
        if not pipeline_name or not pipeline_name.strip():
            raise FabricValidationError(
                "pipeline_name", "empty", "Pipeline name cannot be empty"
            )

        if not source_type or not source_type.strip():
            raise FabricValidationError(
                "source_type", "empty", "Source type cannot be empty"
            )

        if not source_connection_id or not source_connection_id.strip():
            raise FabricValidationError(
                "source_connection_id", "empty", "Source connection ID cannot be empty"
            )

        if not source_schema or not source_schema.strip():
            raise FabricValidationError(
                "source_schema", "empty", "Source schema cannot be empty"
            )

        if not source_table or not source_table.strip():
            raise FabricValidationError(
                "source_table", "empty", "Source table cannot be empty"
            )
    ...
    def _encode_definition(self, definition: Dict[str, Any]) -> str:
        ...
        try:
            ...
            logger.debug(
                f"Pipeline definition encoded: {len(encoded)} base64 characters"
            )
            return encoded
    ...
    def _build_endpoint(
        self, base: str, params: Optional[Dict[str, str]] = None
    ) -> str:
        if params:
            return f"{base}?{urlencode(params)}"
        return base

    def _validate_pipeline_definition_inputs(
    ...
    def create_blank_pipeline(
        ...
        """
        logger.info(
            f"Creating blank pipeline '{pipeline_name}' in workspace {workspace_id}"
        )
        ...
        try:
            ...
            pipeline_definition = {"properties": {"activities": [], "annotations": []}}
            ...
            logger.info(
                f"Successfully created blank pipeline with ID: {created_item.id}"
            )
            return created_item.id
    ...
    def get_pipeline_definition(
        ...
        try:
            ...
            if not pipeline_content_part:
                raise FabricError(
                    "Pipeline definition missing pipeline-content.json part"
                )
    ...
    def add_activity_from_json(
        self, workspace_id: str, pipeline_name: str, activity_json: Dict[str, Any]
    ) -> str:
        ...
        if not isinstance(activity_json, dict):
            raise FabricValidationError(
                "activity_json", "invalid_type", "activity_json must be a dictionary"
            )
    ...
    def add_activity_dependency(
        ...
        try:
            ...
            missing = [name for name in depends_on if name not in activity_names]
            if missing:
                raise FabricValidationError(
                    "depends_on",
                    ", ".join(missing),
    ...
    def _find_dependent_activities(self, activities: list, activity_name: str) -> list:
        dependents = []
        for activity in activities:
    ...
    def get_pipeline_activity_runs(
        ...
        activity_name_filter: Optional[str] = None,
    ) -> Dict[str, Any]:
        """Get activity runs for a pipeline job execution.

        Queries the Fabric queryactivityruns API to retrieve per-activity execution
        details for a given pipeline run.

        Args:
            workspace_id: Workspace ID containing the pipeline
            job_instance_id: Job instance ID from run_on_demand_job or get_job_status
            status_filter: Optional filter by status (Succeeded, Failed, InProgress, Cancelled)
            activity_name_filter: Optional filter by activity name

        Returns:
            Dictionary with activity runs and metadata:
            {
                "activities": [...],  # List of PipelineActivityRun-like dicts
                "activity_count": int,
        ...
        if not job_instance_id or not str(job_instance_id).strip():
            raise FabricValidationError(
                "job_instance_id",
                str(job_instance_id),
                "Job instance ID cannot be empty",
            )

        # Build time window: 7 days ago to 1 day in future
        from datetime import datetime, timedelta, timezone

        now = datetime.now(timezone.utc)
        last_updated_after = (now - timedelta(days=7)).isoformat()
        last_updated_before = (now + timedelta(days=1)).isoformat()
        ...
        try:
            response = self.client.make_api_request(
                "POST",
                f"workspaces/{workspace_id}/datapipelines/pipelineruns/{job_instance_id}/queryactivityruns",
                payload=payload,
            )
            response_data = response.json()
        except FabricAPIError:
            ...
            logger.error(f"Failed to get pipeline activity runs: {exc}")
            raise FabricError(f"Failed to get pipeline activity runs: {exc}")

        # Extract pipeline name from first activity if available
        pipeline_name = None
        raw_activities = response_data.get("value", [])
        if raw_activities:
            pipeline_name = raw_activities[0].get("pipelineName")

        # Transform raw response to curated format
        activities = []
        for raw in raw_activities:
            activity = self._transform_activity_run(raw)
            activities.append(activity)
    ...
    def _transform_activity_run(self, raw: Dict[str, Any]) -> Dict[str, Any]:
        """Transform raw API response to curated activity run format."""
        ...
        return {
            ...
            "duration_ms": raw.get("durationInMs"),
            "start_time": raw.get("activityRunStart"),
            "end_time": raw.get("activityRunEnd"),
            "rows_read": output.get("rowsRead"),
            "rows_written": output.get("rowsCopied"),
