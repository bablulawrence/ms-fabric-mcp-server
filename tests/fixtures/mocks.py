...
class MockResponseFactory:
    ...
    def error(status_code: int, message: str = "Error"):
        ...
        response.json.return_value = {
            "error": {"code": f"Error{status_code}", "message": message}
        }
...
class FabricDataFactory:
    ...
    def item_list(count: int = 5, item_type: str = None) -> Dict[str, Any]:
        ...
        types = (
            [item_type]
            if item_type
            else ["Notebook", "Lakehouse", "Warehouse", "Report", "Dashboard"]
        )
        for i in range(count):
            item_type_val = types[i % len(types)]
            items.append(
                FabricDataFactory.item(f"item-{i}", f"Item {i}", item_type_val)
            )
        return {"value": items}
    ...
    @staticmethod
    def livy_session(
        session_id: int = 1, state: str = "idle", kind: str = "pyspark"
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "appId": (
                f"app-{session_id}"
                if state not in ["not_started", "starting"]
                else None
            ),
            "owner": None,
            "proxyUser": None,
    ...
    def livy_session_list(count: int = 3) -> Dict[str, Any]:
        ...
        return {
            ...
            "total": count,
            "sessions": [
                FabricDataFactory.livy_session(i, "idle") for i in range(count)
            ],
        }

    @staticmethod
    def livy_statement(
        statement_id: int = 0, state: str = "available", output: Dict[str, Any] = None
    ) -> Dict[str, Any]:
    ...
    def job_instance(
        ...
        return {
            ...
            "endTimeUtc": (
                "2025-10-14T10:05:00Z" if status in ["Completed", "Failed"] else None
            ),
        }

    @staticmethod
    def sql_query_result(
        columns: List[str] = None, rows: List[List[Any]] = None
    ) -> tuple:
...
class ServiceMockFactory:
    ...
    def notebook_service(
        mock_client=None, mock_workspace_service=None, mock_item_service=None
    ):
